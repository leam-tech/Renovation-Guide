...
| tableFields | Map&lt;String,List&lt;String&gt;&gt; | no | To get the complete document if a field is of type `Link` \(Another document\) |
...
```javascript
RequestResponse<Item> itemDoc = await getFrappeModelController().setValue(
  "Item",// doctype
  "Item A",// docname
  "item_name"// docfield,
  "Item B Test"// docValue
);
if (itemDoc.isSuccess) {
  // Item Name: Item B Test
  print("Item Name: ${itemDoc.data}");
} else {
  print(itemDoc.error);
}
...
```javascript
Item item = getFrappeModelController().newDoc(
    Item()
  );// no need to await
...
```javascript
Item clonedDoc = await getFrappeModelController().copyDoc(
    getFrappeModelController().newDoc(Item())
  );
...
```javascript
Item amendedDoc = await getFrappeModelController().amendDoc(
    getFrappeModelController().newDoc(Item())
  );
